...
describe("extract integration", function()
  ...
  it("extracts hardcoded JSX text with alias function and skips Trans", function()
    ...
    helpers.with_cwd(root, function()
      ...
      assert.is_true(lines[3]:find('{tr("common:page.hello.world")}', 1, true) ~= nil)
      ...
      assert.are.equal("Hello world", ja_data.page.hello.world)
      assert.are.equal("", en_data.page.hello.world)
    end)
  end)
  ...
  it("applies extraction only within the specified range", function()
    ...
    helpers.with_cwd(root, function()
      ...
      local lines = vim.api.nvim_buf_get_lines(buf, 0, -1, false)
      assert.is_true(lines[4]:find('{t("common:page.first.text")}', 1, true) ~= nil)
